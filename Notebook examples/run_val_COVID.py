...
# Lasso
lasso = LogisticRegression(penalty="l1", class_weight="balanced", max_iter=int(1e6), solver="liblinear", random_state=random_state)
...
# ALasso
alasso = ALogitLasso(penalty="l1", solver="liblinear", max_iter=int(1e6), class_weight='balanced', random_state=random_state)
...
sgl_cv = GridSearchCV(sgl, scoring='roc_auc', param_grid={"alpha": np.logspace(-1, 2, 5), "l1_ratio": [.5, .7, .9]}, cv=chosen_inner_cv, n_jobs=-1)
...
stabl_sgl = clone(stabl).set_params(
    ...
    lambda_grid=[
        ...
        {"alpha": np.logspace(-2, 1, 10), "l1_ratio": [0.7]},
        {"alpha": np.logspace(-2, 1, 10), "l1_ratio": [0.9]}
    ],
    verbose=1)
